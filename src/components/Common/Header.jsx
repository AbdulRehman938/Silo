    ...
    <header onClick={handleHeaderClick} className="fixed top-0 left-0 w-full right-0 z-50 bg-transparent mt-3 md:mt-5">
      <div className="mx-3 lg:mx-10">
        ...
        <div className="flex items-center justify-center gap-6">
          ...
          <nav className="hidden lg:flex items-center justify-center gap-3 xl:gap-4 2xl:gap-6 text-xl font-bold text-black relative">
            ...
            <NavLink
              to="/services"
              className={({ isActive }) => `px-2 xl:px-2.5 2xl:px-1 py-2 xl:py-2.5 2xl:py-3 font-bold whitespace-nowrap transition-colors ${isActive ? 'text-brand font-extrabold' : 'text-brand'}`}
            >
              Services
            </NavLink>
            <NavLink
      ...
      {createPortal(
        <div className={`lg:hidden fixed inset-0 z-40 ${open ? 'pointer-events-auto' : 'pointer-events-none'}`}>
          ...
          <div
            onClick={close}
            className={`absolute inset-0 bg-black/60 transition-opacity duration-200 ${open ? 'opacity-100 pointer-events-auto z-30' : 'opacity-0 pointer-events-none'}`}
          />

          {/* curtain panel: extends from header with smooth curtain animation */}
          <aside
            className={`absolute left-0 right-0 top-16 md:top-20 overflow-hidden transition-all duration-200 ease-out z-40 ${open ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}
